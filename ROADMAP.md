...
## 2. Where we actually are (verified baseline, 2026-09)
...
waves 1–3 replaced the line-scanner with a docutils-fidelity parser, wave 4
...
`objects.inv`, and intersphinx — and wave 4.5 (2026-09-04) added the **Python
domain** (all fourteen `py:*` directives with a real signature grammar, object
registration, py cross-reference resolution, `py-modindex` data) and the
**file-inserting directives** (`include`/`literalinclude`, on a new per-line
provenance layer), each pinned against a real `sphinx-build` 9.1.0 oracle. What
remains **dead code from the binary's execution path** — real modules with real
tests that `sphinx-ultra build` never calls — is the write-side stack the M2 wave-5
HTML writer revives: the Sphinx-mirroring `HTMLBuilder`, the minijinja
...
| Incremental cache | **Working (dependency-driven)** | M1: hits write output, `--clean --incremental` safe, size/expiry knobs plumbed, config-change invalidation, warm-rebuild deadlock fixed, eviction honestly named. M2 wave 4 replaced mtime-only staleness with Sphinx's env-level outdated computation (`BuildEnvironment::get_outdated_files`): added/changed/removed documents plus per-document file dependencies (`src/env/dependencies.rs`, Sphinx's `relfn2path` + asset-collector `note_dependency`). Wave 4.5 ended the images-only limitation: `include` and `literalinclude` record their member files as dependencies, and `env.included` tracks the inclusion graph, so touching an included fragment re-reads the documents that include it. Persisted doctrees carry a magic+version header, so a blob written by an older format is an honest cache miss instead of a silently mis-decoded doctree (bincode has no self-description). Warm-equals-cold is asserted, not assumed — but narrowly: hand-written tests in `tests/env_differential.rs` rebuild their own two- and three-document projects over their own cache and require the warm build's std-domain registries, toctree graph, resolved doctrees and warnings to equal the cold build's. The 28-project oracle corpus is not in that loop; it is built once, cold. **Both cache formats were bumped in wave 4.5** (`DOCTREE_FORMAT_VERSION` 1→2, `ENV_VERSION` 2→3), so the first build after upgrading is a cold one. |
| RST parser | **Working (docutils-fidelity, wired)** | M2 wave 3 (2026-08-13): the binary runs the docutils-fidelity parser (`src/rst/`, sphinx mode); the M1 line-scanner is deleted. Full block+inline grammar (waves 1–2), docutils-exact directive machinery with typed option converters, the docutils built-in directive set, and substitution definitions — zero divergence across a committed 668-case parse-layer differential fixture vs docutils 0.22.4 (`tools/gen_doctree_fixture.py`). Sphinx set (toctree, versionmodified family, seealso, code-block/highlight, only, rst-class, math, index, hlist, glossary, xref + pep/rfc/cve/cwe roles) verified against a second oracle vs a real sphinx-build 9.1.0 read phase (`tools/gen_sphinx_fixture.py`). Wave 4 (2026-08-31) added the std-domain directives — `program`/`option` (incl. `[=value]` and multi-name forms), `envvar`, `confval` with `:type:`/`:default:`, `describe`/`object`, `default-domain` — on a generic **object-description anatomy** (`desc`/`desc_signature`/`desc_name`/`desc_addname`/`desc_annotation`/`desc_content`, the `:no-index:` option family, `PropagateDescDomain`), plus glossary-term ids through Sphinx's `make_id` and index entries following `process_index_entry`. Wave 4.5 (2026-09-04) added the **py domain's fourteen directives** (`py:module`/`currentmodule`/`function`/`class`/`exception`/`method`/`classmethod`/`staticmethod`/`attribute`/`property`/`data`/`decorator`/`decoratormethod`/`type`) with a real signature grammar — a Python expression parser and `ast.unparse` port for defaults and annotations, `pseudo_parse_arglist` for what falls outside that subset, PEP 695 type-parameter lists, and the `DocFieldTransformer` port that renders `:param:`/`:type:`/`:raises:`/`:returns:`/`:rtype:` (which turned out to run for the std kinds too, so an `envvar`'s `:param x:` renders as `Param x` exactly as Sphinx does) — plus **`include` and `literalinclude`** over a new per-line provenance layer: the whole docutils `Include` option set and sphinx's literalinclude reader filter chain (`:lines:`, `:start-after:`/`:end-before:`/`:start-at:`/`:end-at:`, `:pyobject:` via a `DefinitionFinder` tokenizer port, `:prepend:`/`:append:`, `:dedent:`, `:diff:`, `:emphasize-lines:`, `:lineno-match:`, `:tab-width:`, `:encoding:`). The same wave ported `Glossary.run`'s line state machine, so its three misformat diagnostics are recorded in the doctree (not yet printed — the reporter channel is wave-5 work). That fixture now stands at **453 cases, zero divergence**. Still deferred: ifconfig, meta, rst_prolog/epilog/default_role (they want the same per-line provenance layer, which now exists). |
| BuildEnvironment & resolution | **Working** | M2 wave 4 (2026-08-31). The build is a real read → merge → resolve → write pipeline over a serialized `BuildEnvironment` (bincode, `ENV_VERSION`-stamped, per-document doctrees under the `-d` dir). Live: the global toctree graph with relations (parents/prev/next) and Sphinx's consistency warnings (nonexisting/excluded entries, self-reference, circular toctrees, orphans, "document isn't included in any toctree"); section + figure numbering (`numfig`, `numfig_secnum_depth`, `numfig_format`, `:numbered:`); the **std domain** — label/term/option/envvar/confval/program collection and `ref`/`numref`/`doc`/`term`/`option`/`envvar` resolution with Sphinx's duplicate-label and dangling-reference warnings; `nitpick_ignore`/`nitpick_ignore_regex`. Wave 4.5 (2026-09-04) added the **py domain**: object and module registration into `domaindata['py']` (insertion-ordered, with aliased entries, `duplicate object description of …` and `more than one target found for cross-reference …` warnings), py cross-reference resolution for `:py:func:`/`:py:class:`/`:py:meth:`/`:py:mod:`/… — which **re-scoped the nitpicky skip notice** (its python-domain population is gone; it survives as `N cross-domain reference(s) not validated` for the `c`/`cpp`/`js`/`rst` domains M5 adds) — `:any:` resolution as the domainless role it actually is, the `builtin_resolver` fallback, and toc object entries. Evidence: an environment-layer differential oracle (`tools/gen_env_fixture.py`, `tests/env_differential.rs`) that builds **28 multi-document projects** (83 documents) with a real `sphinx-build` 9.1.0 and compares every recorded key — `tocs`, `toc_num_entries`, `toctree_includes`, `files_to_rebuild`, relations, `toc_secnumbers`/`toc_fignumbers`, the std registries, index entries, genindex, the whole warning stream, and each document's fully cross-reference-resolved doctree — at **zero divergence** over one cold build per project (warm-equals-cold is the separate, narrower claim above). Strict, self-cleaning exemption tables remain, each pinned to the work that closes it: 23 of 28 projects' warning streams match byte-for-byte (3 differ only by `image file not readable`, 1 by the write-phase circular-toctree warning, 1 because its whole oracle warning set is docutils *reporter* output that this crate keeps in-tree only), and 34 of 83 documents' resolved doctrees match byte-for-byte (43 are skipped wholesale — an unresolved `toctree` node, an image without `candidates`, an unapplied `PropagateTargets`, or the `orphan` docinfo field list Sphinx removes — and 6 are compared in full except for the highlight stamp; the arithmetic is computed from the tables and asserted by the test itself). A sixth table added in wave 4.5, `KNOWN_HIGHLIGHT_STAMP_GAPS`, is narrower than the rest: it still compares the entire tree and forgives nothing but the *presence* of a `HighlightLanguageTransform` attribute we do not emit at all — never a directive-set `linenos="1"` — while the `inc_highlight` project compares directive-set `language`/`force`/`linenos` at full strength. An exemption whose project *stops* diverging fails the test, so the tables shrink on their own. |
| genindex & domain indices | **Working (data)** | M2 wave 4: `index` directive/role entries are collected per Sphinx's `IndexDomain.process_doc` (incl. the `invalid … index entry` warning) and assembled by a port of `IndexEntries.create_index` — single/pair/triple/see/seealso, `!main` promotion, Symbols/`_` grouping, sub-entry ordering, and the "dropped index entry" notice. Pinned by the env oracle's `genindex` key across the corpus and by `src/env/genindex.rs`'s own tests. **No `genindex.html` is written** — the page render needs the wave-5 HTML writer. Wave 4.5 added the **py-modindex** data — `PythonModuleIndex.generate`'s grouping, `modindex_common_prefix` stripping, the collapse/deprecated/platform/synopsis columns — recorded by the env oracle. Its page, like `genindex.html`, needs the wave-5 HTML writer. |
...
| Validation systems | **Two of three live** | Directive/role validation runs in every build (default on, false-positive heuristics fixed/demoted, Unknown silent). Wave 4.5 audited every built-in validator against the parser's own probe-verified `option_spec` tables, in both directions, and fixed six drifts between the two: four of them invented `Unknown option '…'` warnings Sphinx has no counterpart for — the class of bug that fails `-W` on a project `sphinx-build` accepts — one (`include`'s) was latent, because that validator never consults its own option list, and one (`figure`'s missing `figname`) was a gap in the PARSER's table that the audit compared against. Two mechanical tests now hold the two lists together for all ten validators. Cross-reference validation is no longer the M1 heuristic scanner: wave 4 replaced it with the std domain + Sphinx's own resolution pass, and with it Sphinx's `warn_dangling` semantics — **all seven `warn_dangling` std reftypes now warn about broken targets without `-n`**: `ref`, `numref`, `doc`, `term`, `keyword`, `option`, `confval` (`domains/std/__init__.py:748-766`), with `-n`/`nitpicky` reserved for everything else. Wave 4.5 added the py domain's resolution and **re-scoped the "skipping python-domain references" notice** (it survives as `N cross-domain reference(s) not validated` for the `c`/`cpp`/`js`/`rst` domains M5 adds): `:py:*:` roles, `:any:`, duplicate object descriptions and ambiguous cross-references all warn now, `-n` included. That is a behavior change for existing projects (see CHANGELOG). The M1 `src/domains/` module was deleted once nothing called it. Constraint engine stays library-only until sphinx-needs items exist (M4) — no `ContentItem` producer yet; its always-success placeholder trait impls were deleted so future wiring can't silently no-op. |
...
**Wave 4.5 delivered 2026-09-04**, all three bullets: the py domain (fourteen
directives with a real signature grammar, registration, cross-reference resolution)
and the ten-key object-signature config family; `py-modindex` data; and
`include`/`literalinclude` over a new per-line provenance layer, which ends the
...
83 documents and the read-phase doctree oracle at 453 cases, both zero divergence.
