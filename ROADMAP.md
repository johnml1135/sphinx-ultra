...
| Validation systems | **Two of three live** | Directive/role validation runs in every build (default on, false-positive heuristics fixed/demoted, Unknown silent). Wave 4.5 audited every built-in validator against the parser's own probe-verified `option_spec` tables, in both directions, and fixed five drifts that invented `Unknown option '…'` warnings Sphinx has no counterpart for — the class of bug that fails `-W` on a project `sphinx-build` accepts. Two mechanical tests now hold the two lists together for all ten validators. Cross-reference validation is no longer the M1 heuristic scanner: wave 4 replaced it with the std domain + Sphinx's own resolution pass, and with it Sphinx's `warn_dangling` semantics — **all seven `warn_dangling` std reftypes now warn about broken targets without `-n`**: `ref`, `numref`, `doc`, `term`, `keyword`, `option`, `confval` (`domains/std/__init__.py:748-766`), with `-n`/`nitpicky` reserved for everything else. Wave 4.5 added the py domain's resolution and **retired the "skipping python-domain references" notice**: `:py:*:` roles, `:any:`, duplicate object descriptions and ambiguous cross-references all warn now, `-n` included. That is a behavior change for existing projects (see CHANGELOG). The M1 `src/domains/` module was deleted once nothing called it. Constraint engine stays library-only until sphinx-needs items exist (M4) — no `ContentItem` producer yet; its always-success placeholder trait impls were deleted so future wiring can't silently no-op. |
...
documents and the read-phase doctree oracle at 426 cases, both zero divergence.
