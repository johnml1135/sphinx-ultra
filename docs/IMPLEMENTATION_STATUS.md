...
| Directive/role validation in the build | ✅ | Wired wave 4: runs on every build (`validate_directives`, default on; `-D validate_directives=0` disables). Findings surface as warnings with file:line through the standard `-W`/`-w` pipeline. Unknown directives/roles stay silent (10+10 validators cover a fraction of Sphinx). False-positive heuristics fixed/demoted: `.. note:: inline` is content not arguments; bare `code-block`, spaces/uppercase in `:ref:` labels, relative `:doc:` paths, kbd/menuselection styles all accepted. **M2 wave 4.5** audited all ten validators against the parser's own probe-verified `option_spec` tables, in both directions, after the env oracle caught one drift in the wild (`Unknown option 'lines'` on a `literalinclude`). Five more were found and fixed: `code-block`'s `force` and `class`, `figure`'s `figwidth`/`figclass` (which warned under the *image* directive's name), `image`/`figure`'s `loading`, and `include`'s `parser`/`class`/`name` — plus `literalinclude`'s `start-line`/`end-line`, which it advertised although sphinx's `LiteralInclude` has neither. Each list is now one shared const, and two mechanical tests keep the lists and the `validate` match in step for every validator. |
...
**973 tests, all passing** (as of M2 wave 4.5). Every generator below is pinned to
...
| Suite | Status |
|---|---|
| Unit tests (lib + bin) | ✅ 835 passing (828 lib + 7 bin) |
...
| Sphinx doctree differential suite (real read phase) | ✅ 426 generated cases vs a `sphinx-build` 9.1.0 read phase, zero divergence; regenerate with `uv run --python 3.12 --with 'sphinx==9.1.0' --with 'docutils==0.22.4' python tools/gen_sphinx_fixture.py` |
...
| Property tests (`tests/rst_proptest.rs`) | ✅ 14 passing — the parser never panics on arbitrary, multiline, multibyte or deeply nested input, and (wave 4.5) on arbitrary py and std object signatures, arbitrary annotations through `parse_annotation`, and arbitrary `include`/`literalinclude` option blocks and file arguments against a real scratch srcdir. Also green at `PROPTEST_CASES=2048` |
