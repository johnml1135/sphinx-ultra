...
**1023 tests, all passing** (as of M2 wave 4.5, panel fix round B). That is
what `cargo test` reports across its thirteen targets — 867 lib + 7 bin + 149
...
returns 1021 — two short, because `tests/inventory_roundtrip.rs` writes two of
...
| Suite | Status |
|---|---|
| Unit tests (lib + bin) | ✅ 874 passing (867 lib + 7 bin) |
...
- Expressions outside the supported subset take the `pseudo_parse_arglist`
  ...
  in a call render fine under sphinx, so there the difference is a different
  parameter list on our side with no warning on either — a `lambda` default in
  particular is comma-split by the fallback. Complex defaults differ textually
  (`1 + 2j` vs `1+2j`).
