...
**1027 tests, all passing** (as of M2 wave 4.5, panel fix round C). That is
what `cargo test` reports across its thirteen targets — 871 lib + 7 bin + 149
...
returns 1025 — two short, because `tests/inventory_roundtrip.rs` writes two of
...
| Suite | Status |
|---|---|
| Unit tests (lib + bin) | ✅ 878 passing (871 lib + 7 bin) |
...
| Doctree differential suite (docutils parse layer) | ✅ 668 generated cases vs docutils 0.22.4, zero divergence; regenerate with `PYTHONNOUSERSITE=1 uv run --python 3.12 --with docutils==0.22.4 python tools/gen_doctree_fixture.py` (the flag is not optional — `uv run` keeps user site-packages on `sys.path`, and a user-site Pygments there silently re-records every `code:: python` case as tokenized output) |
| Sphinx doctree differential suite (real read phase) | ✅ 458 generated cases vs a `sphinx-build` 9.1.0 read phase, zero divergence; regenerate with `PYTHONNOUSERSITE=1 uv run --python 3.12 --with 'sphinx==9.1.0' --with 'docutils==0.22.4' python tools/gen_sphinx_fixture.py` |
